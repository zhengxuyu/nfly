...
| Fly, frozen + DAgger, whole CNS | supervised (`scripts/dagger_pong.py --subset all`) | readout on descending + motor neurons (VNC included), 64-unit MLP head | 96k labelled | +10.3 | | the VNC adds no nonlinearity the head needs; linear head on the same readout -19.2 |
| Fly, from the DAgger head + PPO | simple PPO | brain frozen; separate readout critic / shared trunk / pixel critic (lr bug) | 174k / 128k / 72k | -17.6 / -20.6 / -17.6 | | none of the three critics learns to predict the return; the policy gradient stays noise (ablation, section 16) |
